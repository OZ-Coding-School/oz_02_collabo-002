...
const DesignPreviewBox = () => {
  ...
  let imageFile: { imageUrl: string; imageName: string }[] = [];
  const router = useRouter();

  const imagePath = {
    whiteTShirt: '/images/t-shirt_white.png',
    blackTShirt: '/images/t-shirt_black.png',
    aiImage1:
  ...
  useEffect(() => {
    if (canvasRef?.current) {
      ...
      imageFile.push({ imageUrl: imageDataUrl, imageName: 'ai생성이미지1' });
      imageFile.push({ imageUrl: imageDataUrl, imageName: 'ai생성이미지2' });
      console.log(imageFile);
    }
  }, [canvasRef, selectedColor, imageFile]);
